...
#[cfg(verus_keep_ghost)]
use crate::specs::core_specs::word_from_bytes;
#[cfg(verus_keep_ghost)]
use crate::specs::core_specs::words_from_bytes_to_nat;
#[cfg(verus_keep_ghost)]
use crate::specs::core_specs::bytes_seq_to_nat;
#[cfg(verus_keep_ghost)]
use crate::specs::core_specs::bytes_seq_to_nat_clear_aux;
...
impl Scalar52 {
    ...
    pub fn from_bytes(bytes: &[u8; 32]) -> (s: Scalar52)
        ensures
            bytes_to_nat(bytes) == scalar52_to_nat(&s.limbs),
            limbs_bounded(&s),
    {
        ...
        proof {
            ...
            assert(bytes_to_nat(bytes) == scalar52_to_nat(&s.limbs));
        }
    ...
    pub fn from_bytes_wide(bytes: &[u8; 64]) -> (s: Scalar52)
        ensures
            limbs_bounded(&s),
            scalar52_to_nat(&s.limbs) % group_order() == bytes_wide_to_nat(bytes) % group_order(),
            // VERIFICATION NOTE: Result is canonical
            scalar52_to_nat(&s.limbs) < group_order(),
    {
        ...
        assert(scalar52_to_nat(&hi_raw.limbs) == high_expr);
        ...
        assert(scalar52_to_nat(&lo_raw.limbs) == wide_input % pow2(260)) by {
        ...
        assert(scalar52_to_nat(&hi_raw.limbs) == wide_input / pow2(260)) by {
        ...
        proof {
            let ghost lo_before_nat = scalar52_to_nat(&lo_raw.limbs);
            let ghost lo_after_nat = scalar52_to_nat(&lo.limbs);
            let ghost r_nat = scalar52_to_nat(&constants::R.limbs);
            ...
            let ghost hi_before_nat = scalar52_to_nat(&hi_raw.limbs);
            let ghost hi_after_nat = scalar52_to_nat(&hi.limbs);
            let ghost rr_nat = scalar52_to_nat(&constants::RR.limbs);
        ...
        proof {
            lemma_montgomery_reduced_sum_congruent(
                scalar52_to_nat(&result.limbs),
                scalar52_to_nat(&hi.limbs),
                scalar52_to_nat(&lo.limbs),
                scalar52_to_nat(&hi_raw.limbs),
                scalar52_to_nat(&lo_raw.limbs),
                wide_input,
            );

            lemma_cancel_mul_pow2_mod(scalar52_to_nat(&result.limbs), wide_input, montgomery_radix());
        }
    ...
    pub fn as_bytes(self) -> (s: [u8; 32])
        requires
            limbs_bounded(&self),
        ensures
            bytes_to_nat(&s) == scalar52_to_nat(&self.limbs) % pow2(256),
    {
        let mut s = [0u8;32];
    ...
    pub fn add(a: &Scalar52, b: &Scalar52) -> (s: Scalar52)
        requires
            limbs_bounded(a),
            limbs_bounded(b),
            scalar52_to_nat(&a.limbs) < group_order(),
            scalar52_to_nat(&b.limbs) < group_order(),
        ensures
            scalar52_to_nat(&s.limbs) == (scalar52_to_nat(&a.limbs) + scalar52_to_nat(&b.limbs)) % group_order(),
            // VERIFICATION NOTE: Result is canonical
            scalar52_to_nat(&s.limbs) < group_order(),
            ...
            limbs_bounded(&s),
    {
        ...
        assert(group_order() > scalar52_to_nat(&sum.limbs) - group_order() >= -group_order());
        proof {
            lemma_l_equals_group_order();
        }
        proof {
            lemma_mod_sub_multiples_vanish(scalar52_to_nat(&sum.limbs) as int, group_order() as int);
        }
    ...
    pub fn sub_new(a: &Scalar52, b: &Scalar52) -> (s: Scalar52)
        requires
            limbs_bounded(a),
            limbs_bounded(b),
            -group_order() <= scalar52_to_nat(&a.limbs) - scalar52_to_nat(&b.limbs) < group_order(),
        ensures
            scalar52_to_nat(&s.limbs) == (scalar52_to_nat(&a.limbs) - scalar52_to_nat(&b.limbs)) % (group_order() as int),
    {
    ...
    pub(crate) fn conditional_add_l(&mut self, condition: Choice) -> (carry: u64)
        requires
            ...
            scalar52_to_nat(&old(self).limbs) + group_order() < pow2(260),
        ensures
    // The mathematical value modulo group_order doesn't change (since L = group_order)

            scalar52_to_nat(&self.limbs) % group_order() == scalar52_to_nat(&old(self).limbs) % group_order(),
            // VERIFICATION NOTE: expression below unsupported by Verus
            //limbs_bounded(&self),
            // Meaning of conditional addition
            super::subtle_assumes::choice_is_true(condition) ==> scalar52_to_nat(&self.limbs) == scalar52_to_nat(
                &old(self).limbs,
            ) + group_order(),
            !super::subtle_assumes::choice_is_true(condition) ==> scalar52_to_nat(&self.limbs) == scalar52_to_nat(
                &old(self).limbs,
            ),
    {
        ...
        proof {
            ...
            assume(scalar52_to_nat(&self.limbs) % group_order() == scalar52_to_nat(&old(self).limbs) % group_order());
            //   assume(limbs_bounded(&self));
            assume(super::subtle_assumes::choice_is_true(condition) ==> scalar52_to_nat(&self.limbs)
                == scalar52_to_nat(&old(self).limbs) + group_order());
            assume(!super::subtle_assumes::choice_is_true(condition) ==> scalar52_to_nat(&self.limbs)
                == scalar52_to_nat(&old(self).limbs));
        }
    ...
    pub fn sub(a: &Scalar52, b: &Scalar52) -> (s: Scalar52)
        requires
            ...
            // scalar52_to_nat(&a.limbs) >= scalar52_to_nat(&b.limbs) ==> scalar52_to_nat(&s.limbs) == scalar52_to_nat(&a.limbs) - scalar52_to_nat(&b.limbs),
            // scalar52_to_nat(&a.limbs) < scalar52_to_nat(&b.limbs) ==> scalar52_to_nat(&s.limbs) == (scalar52_to_nat(&a.limbs) - scalar52_to_nat(&b.limbs) + pow2(260) + group_order()) % (pow2(260) as int),
    ...
    pub(crate) fn mul_internal(a: &Scalar52, b: &Scalar52) -> (z: [u128; 9])
        requires
            ...
            limbs_bounded(b),
        ensures
            slice128_to_nat(&z) == scalar52_to_nat(&a.limbs) * scalar52_to_nat(&b.limbs),
    ...
    pub(crate) fn square_internal(a: &Scalar52) -> (z: [u128; 9])
        requires
            limbs_bounded(a),
        ensures
            slice128_to_nat(&z) == scalar52_to_nat(&a.limbs) * scalar52_to_nat(&a.limbs),
        ...
        ensures
            (exists|bounded1: &Scalar52, bounded2: &Scalar52|
                limbs_bounded(bounded1) && limbs_bounded(bounded2) && spec_mul_internal(
                    bounded1,
                    bounded2,
                ) == limbs) ==> ((scalar52_to_nat(&result.limbs) * montgomery_radix()) % group_order()
            ...
            (exists|bounded: &Scalar52, canonical: &Scalar52|
                limbs_bounded(bounded) && limbs_bounded(canonical) && scalar52_to_nat(&canonical.limbs)
                    < group_order() && spec_mul_internal(bounded, canonical) == limbs) ==> scalar52_to_nat(
                &result.limbs,
            ) < group_order(),
    {
    ...
    pub fn mul(a: &Scalar52, b: &Scalar52) -> (result: Scalar52)
        requires
            ...
            limbs_bounded(b),
        ensures
            scalar52_to_nat(&result.limbs) % group_order() == (scalar52_to_nat(&a.limbs) * scalar52_to_nat(&b.limbs))
                % group_order(),
            // VER NOTE: Result has bounded limbs from montgomery_reduce
            limbs_bounded(&result),
            // VER NOTE: Result is canonical from montgomery_reduce
            scalar52_to_nat(&result.limbs) < group_order(),
    {
        proof {
            lemma_rr_limbs_bounded();
        }
        ...
        assert((scalar52_to_nat(&ab.limbs) * montgomery_radix()) % group_order() == (scalar52_to_nat(&a.limbs)
            * scalar52_to_nat(&b.limbs)) % group_order());
        ...
        assert((scalar52_to_nat(&result.limbs) * montgomery_radix()) % group_order() == (scalar52_to_nat(&ab.limbs)
            * scalar52_to_nat(&constants::RR.limbs)) % group_order());
        ...
        proof {
            ...
            lemma_cancel_mul_montgomery_mod(
                scalar52_to_nat(&result.limbs),
                scalar52_to_nat(&ab.limbs),
                scalar52_to_nat(&constants::RR.limbs),
            );

            // 3. Since result < group_order (from montgomery_reduce), result % L == result
            lemma_small_mod(scalar52_to_nat(&result.limbs), group_order());
        }
    ...
    pub fn square(&self) -> (result: Scalar52)
        requires
            limbs_bounded(self),
        ensures
            scalar52_to_nat(&result.limbs) == (scalar52_to_nat(&self.limbs) * scalar52_to_nat(&self.limbs)) % group_order(),
    {
        proof {
            lemma_rr_limbs_bounded();
        }
        ...
        assert((scalar52_to_nat(&aa.limbs) * montgomery_radix()) % group_order() == (scalar52_to_nat(&self.limbs)
            * scalar52_to_nat(&self.limbs)) % group_order());
        ...
        assert((scalar52_to_nat(&result.limbs) * montgomery_radix()) % group_order() == (scalar52_to_nat(&aa.limbs)
            * scalar52_to_nat(&constants::RR.limbs)) % group_order());

        proof {
            // 1. prove (scalar52_to_nat(&constants::RR.limbs) % group_order() == (montgomery_radix()*montgomery_radix()) % group_order()
            lemma_rr_equals_spec(constants::RR);

            // 2. Reduce to (scalar52_to_nat(&result.limbs)) % group_order() == (scalar52_to_nat(&self.limbs) * scalar52_to_nat(&self.limbs)) % group_order()
            lemma_cancel_mul_montgomery_mod(
                scalar52_to_nat(&result.limbs),
                scalar52_to_nat(&aa.limbs),
                scalar52_to_nat(&constants::RR.limbs),
            );

            // 3. allows us to assert (scalar52_to_nat(&result.limbs)) % group_order() == (scalar52_to_nat(&result.limbs))
            //  true from montgomery_reduce postcondition
            lemma_small_mod((scalar52_to_nat(&result.limbs)), group_order())
        }

        assert((scalar52_to_nat(&result.limbs)) % group_order() == (scalar52_to_nat(&aa.limbs) * montgomery_radix())
            % group_order());
    ...
    pub fn montgomery_mul(a: &Scalar52, b: &Scalar52) -> (result: Scalar52)
        ...
        ensures
            limbs_bounded(&result),
            (scalar52_to_nat(&result.limbs) * montgomery_radix()) % group_order() == (scalar52_to_nat(&a.limbs)
                * scalar52_to_nat(&b.limbs)) % group_order(),
    {
    ...
    pub fn montgomery_square(&self) -> (result: Scalar52)
        ...
        ensures
            limbs_bounded(&result),
            (scalar52_to_nat(&result.limbs) * montgomery_radix()) % group_order() == (scalar52_to_nat(&self.limbs)
                * scalar52_to_nat(&self.limbs)) % group_order(),
    {
    ...
    pub fn as_montgomery(&self) -> (result: Scalar52)
        ...
        ensures
            limbs_bounded(&result),
            #[trigger] (scalar52_to_nat(&result.limbs) % group_order()) == #[trigger] ((scalar52_to_nat(&self.limbs)
                * montgomery_radix()) % group_order()),
    {
        ...
        proof {
            ...
            // (scalar52_to_nat(&result.limbs) * montgomery_radix()) % group_order() ==
            // (scalar52_to_nat(&self.limbs) * scalar52_to_nat(&constants::RR.limbs)) % group_order()
...
pub mod test {
    ...
    /// i.e. the postcondition `scalar52_to_nat(&s.limbs) < group_order()` may not hold
    ///
    ...
    fn from_bytes_non_canonical_example() {
        ...
        // OLD Postcondition 3: scalar52_to_nat(&s.limbs) < group_order() - DOES NOT HOLD
        assert!(
            &result_nat >= &l,
    ...
    proptest! {
        ...
        /// 1. bytes_to_nat(bytes) == scalar52_to_nat(&s.limbs)
        /// 2. limbs_bounded(&s)
        #[test]
        fn prop_from_bytes(bytes in prop::array::uniform32(any::<u8>())) {
            ...
            // Postcondition 1: bytes_to_nat(bytes) == scalar52_to_nat(&s.limbs)
            prop_assert_eq!(bytes_nat, result_nat,
                "from_bytes spec violated: bytes_to_nat(bytes) != scalar52_to_nat(&s.limbs)");
