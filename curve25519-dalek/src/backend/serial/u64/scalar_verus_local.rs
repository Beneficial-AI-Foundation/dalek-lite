...
verus! {
    ...
    // Verus-compatible Choice type and ConditionallySelectable trait
    // This replaces the subtle crate functionality for Verus verification
    ...
    pub trait ConditionallySelectable {
        fn conditional_select(a: &Self, b: &Self, choice: Choice) -> Self where Self: core::marker::Sized;
    }

    impl ConditionallySelectable for u64 {
        #[verifier::external_body]
        fn conditional_select(a: &u64, b: &u64, choice: Choice) -> u64 {
            if choice.value == 0 { *b } else { *a }
        }
    }
