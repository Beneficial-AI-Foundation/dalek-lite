...
impl<'a, 'b> Mul<&'b EdwardsPoint> for &'a Scalar {
    ...
    /// For scalar multiplication of a basepoint,
    /// `EdwardsBasepointTable` is approximately 4x faster.
    #[verifier::external_body]  // Trivial forwarding to EdwardsPoint * Scalar
...
/// This allows Verus to see and use this type in verified code
...
impl EdwardsPoint {
    ...
    #[verifier::external_body]  // Uses macro-generated multiplication outside verus
...
// Manual implementations of Scalar * EdwardsPoint variants
// These are normally generated by define_mul_variants! macro, but we define them
// manually inside verus! block so Verus can see them for verification
// They transparently delegate to the reference implementation (&Scalar * &EdwardsPoint)
// which is now also inside the verus block
// NOTE: MulSpecImpl for EdwardsPoint * Scalar and Scalar * EdwardsPoint variants moved to mul_specs.rs
impl<'b> Mul<&'b EdwardsPoint> for Scalar {
    type Output = EdwardsPoint;

    #[verifier::external_body]  // Trivial forwarding to &Scalar * &EdwardsPoint
...
// Manual implementations of owned-type Scalar * EdwardsBasepointTable variants
// The reference implementation (&Scalar * &EdwardsBasepointTable) is created by the
// impl_basepoint_table! macro outside the verus block, so we use external_body to
// delegate to it. We only add the owned variants here.
