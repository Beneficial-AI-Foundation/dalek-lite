...
///
/// NOTE: Uses lemma_mod_factor_divisible and lemma_mod_sum_both_divisible from common_lemmas
    ...
    decreases (32 - n),
{
    ...
    if n >= 32 {
        ...
        assert((bytes[n as int] as nat * d) % d == 0);
        assert(term1 % d == 0);

        // Part 2: u8_32_as_nat_rec(bytes, n+1) is divisible by pow2((n+1)*8) by IH
        lemma_rec_suffix_divisible(bytes, (n + 1) as nat);
        assert(u8_32_as_nat_rec(bytes, (n + 1) as nat) % pow2(((n + 1) * 8) as nat) == 0);

        // Since pow2((n+1)*8) == pow2(n*8) * pow2(8), it's also divisible by pow2(n*8)
        lemma_pow2_adds((n * 8) as nat, 8);
        assert(pow2(((n + 1) * 8) as nat) == pow2((n * 8) as nat) * pow2(8));

        // If x % (d * k) == 0, then x % d == 0
        lemma_mod_factor_divisible(u8_32_as_nat_rec(bytes, (n + 1) as nat), d, pow2(8));
        ...
        u8_32_as_nat(bytes) % pow2(n * 8) == as_nat_prefix(bytes, n),
{
    ...
    if n == 0 {
        ...
        assert(as_nat_prefix(bytes, n) < pow2((n * 8) as nat));

        // Show that u8_32_as_nat_rec(bytes, n) is divisible by pow2(n*8)
        lemma_rec_suffix_divisible(bytes, n);
        assert(u8_32_as_nat_rec(bytes, n) % pow2((n * 8) as nat) == 0);
        ...
        (as_nat_prefix(bytes, i + 1) / pow2(i * 8)) % pow2(8) == bytes[i as int] as nat,
{
    ...
    assert(as_nat_prefix(bytes, i) < pow2((i * 8) as nat));
    ...
    // Use division algebra
    lemma_div_aligned_sum(low, byte_val, d);
    ...
    assert(byte_val % pow2(8) == byte_val);
}
        ...
        bytes[i as int] as nat == (u8_32_as_nat(bytes) / pow2(i * 8)) % pow2(8),
{
    ...
    assert(u8_32_as_nat(bytes) % pow2((i + 1) * 8) == as_nat_prefix(bytes, i + 1));

    // Step 4: Apply division extraction lemma
    lemma_as_nat_prefix_div_extracts_byte(bytes, i);
    assert((as_nat_prefix(bytes, i + 1) / pow2(i * 8)) % pow2(8) == bytes[i as int] as nat);
