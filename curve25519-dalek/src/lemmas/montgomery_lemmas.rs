...
pub(crate) proof fn lemma_rr_equals_radix_squared()
    ensures
        scalar52_as_nat(&constants::RR) % group_order() == (montgomery_radix() * montgomery_radix())
            % group_order(),
{
    // Enable conversion between scalar52_as_nat and five_limbs_to_nat_aux representations
    ...
    //
    // Therefore: scalar52_as_nat(RR.limbs) % L == (R * R) % L  ✓
